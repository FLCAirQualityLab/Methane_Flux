...
# -------------------------------------------------------------- Calibration code begins on line 192 -----------------------------------------------------------
...
endTime = pd.to_datetime("2025-07-01 16:17:00")        # Must be in [yyyy-mm-dd hh-mm-ss] format
...
INPUTS:
    ...
    USER INPUTS:
        setpointerror: Error in setpoint calculations
    
OUTPUTS:
    Plot of regression data response in methane along with delivered setpoints.
    Plot of regression data plotted against delivered methane concentration (1 to 1 plot).
    3 regression models — reglow.plk, regmid.plk, reghigh.plk — found in /Coeffiecients.
"""
...
#ambTrain = setpointavg[(setpointavg["Setpoint"] <= 20) & (setpointavg["Setpoint"] >= 0)]
...
#ambTest = setpointavg[(setpointavg["Setpoint"] <= 20) & (setpointavg["Setpoint"] >= 0)]
...
# --- Model Assignments ---
low_reg = clone(polynomial_model_deg2)
med_reg = clone(polynomial_model_deg2)
#amb_reg = LinearRegression()
...
Xlow = lowTrain[["BO TGS2600", "BO TGS2611", "BO Temperature", "BO Humidity"]].copy()
#Xamb = ambTrain[["BO TGS2600", "BO TGS2611", "BO TGS2602", "BO Temperature", "BO Humidity"]].copy()
Xmed = medTrain[["BO TGS2600", "BO TGS2611", "SGX_Digital", "BO Temperature", "BO Humidity"]].copy()
Xhigh = highTrain[["BO TGS2611", "SGX_Digital", "BO Temperature", "BO Humidity"]].copy()

Xlo = lowTest[["BO TGS2600", "BO TGS2611", "BO Temperature", "BO Humidity"]].copy()
#xAmb = ambTest[["BO TGS2600", "BO TGS2611", "BO TGS2602", "BO Temperature", "BO Humidity"]].copy()
Xmid = medTest[["BO TGS2600", "BO TGS2611", "SGX_Digital", "BO Temperature", "BO Humidity"]].copy()
Xhi = highTest[["BO TGS2611", "SGX_Digital", "BO Temperature", "BO Humidity"]].copy()

# 2. RENAME the columns on the DataFrames *BEFORE* fitting the model.
# This is the crucial fix. The model will now learn the clean names.
rename_dict = {
...
#Xamb.rename(columns=rename_dict, inplace=True)
...
# 3. Fit the regression models using the DataFrames with the NEW, clean names
...
print("\n\n------ R² & RMS ERROR REPORT ------")
print(f"Low Range Model (Poly Deg 2) --> RMS: {RMSlow:.4f} [ppm], R²: {r2_low:.4f}")
print(f"Medium Range Model (Poly Deg 2) -> RMS: {RMSmid:.4f} [ppm], R²: {r2_med:.4f}")
print(f"High Range Model (Poly Deg 3)--> RMS: {RMShigh:.4f} [ppm], R²: {r2_high:.4f}")
...
def format_equation(coefs, features, intercept, threshold=0.001, sig_figs=4):
    terms = []
    ...
    return f"Setpoint = {full_equation.strip()}"
...
for name, data_object in [('lowreg', low_reg), ('midreg', med_reg), ('highreg', high_reg)]:
    filepath = os.path.join("Coefficients", f"{name}_Boron{pcbNum}.pkl")
    with open(filepath, "wb") as f:
        pickle.dump(data_object, f)

# Loop to save the text files
for name, value in [('L', RMSlow), ('M', RMSmid), ('H', RMShigh)]:
    filepath = os.path.join("Coefficients", f"B_RMSE_{name}_Boron{pcbNum}.txt")
...
def sensAccuracy():
    ...
    # Define the padding amount (e.g., 5% of the core data range)  <-- does not work but ok
    ...
    # --- 2. Build the scale using these padded boundaries ---
    ...
    # 3. Define the SHAPE-AWARE transformation functions based on the padded anchors.
    ...
    # 4. Apply the SAME universal padded scale to BOTH axes.
    ...
    # --- 5. Plot the data (NO set_xlim/ylim NEEDED) ---
...
def modelAccuracy():
    ...
    # Plotting data
    #ax.plot(reg_model_diff_amb.index, reg_model_diff_amb["Predicted value"], label = '0-20 Range Prediction', color = "skyblue")   # Plotting low range 
    #ax.fill_between(reg_model_diff_amb.index, reg_model_diff_amb["Predicted value"] - RMSamb,
    #                 reg_model_diff_amb["Predicted value"] + RMSamb,color = 'skyblue', alpha = 0.25, zorder=-1)     # Filling in 
...
fig5, ax5 = plt.subplots(dpi=600, figsize = (5, 6))
...
# RMS & R² stats
ax5.text(0.5, 0.89,
         f"""Low Range Model ---> RMS: {RMSlow:.4f} [ppm], R²: {r2_low:.4f}
Medium Range Model ---> RMS: {RMSmid:.4f} [ppm], R²: {r2_med:.4f}
High Range Model ---> RMS: {RMShigh:.4f} [ppm], R²: {r2_high:.4f}
Combined Model ---> RMS: {RMS:.4f} [ppm], R²: {r2_combined:.4f}""",
...
draw_wrapped_text(ax5, medEQ, 0.46, "Medium EQ:", font_size=8)
draw_wrapped_text(ax5, highEQ, 0.19, "High EQ:", font_size=8)


#%% Recreating correlogram with pairgrid...

# Choosing which MOX columns to remove from the correlogram
...
#%% Residuals plot...
# ==================================================================
#         Residuals Plot (Trained & Tested on Full Dataset)
# ==================================================================
print("\nGenerating residuals plot for the full-dataset calibration...")

# --- Step 1: Consolidate model results and calculate residuals ---
# Your script already produced the necessary dataframes (reg_model_diff_low, etc.)
# We will combine them into a single dataframe suitable for plotting.
...
# --- Step 2: Define and apply the custom x-axis scale with PADDING ---
...
# ==================================================================
print("\nGenerating residual error vs. temperature plot...")

# --- Step 1: Consolidate results, calculate residuals, and add Temperature ---
# We will create a new dataframe that includes the temperature for each setpoint.
# The indices from `reg_model_diff_*` align with the `*Test` dataframes,
# which are slices of the `setpointavg` dataframe containing the temperature.
...
# ==================================================================
print("\nGenerating residual error vs. humidity plot...")

# --- Step 1: Consolidate results, calculate residuals, and add Humidity ---
# This step is identical to the last one, but we grab 'BO Humidity' instead.
...
#     lE, lT = startLoading(message="Generating PDF")
