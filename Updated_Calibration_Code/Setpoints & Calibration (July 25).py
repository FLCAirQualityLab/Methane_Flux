...
defaultPath = "MetaData_071125.csv"

pcbNum = 9              # What Boron sensor are you using?

# Setting start & end times
startTime = pd.to_datetime("2025-07-11 10:18:00")      # Must be within selected dataset
endTime = pd.to_datetime("2025-07-17 14:30:00")        # Must be in [yyyy-mm-dd hh-mm-ss] format
...
MOX = sheetPuller("Data", f"Boron_{pcbNum}")
...
try:
    ...
    lowTrain = setpointavg[(setpointavg["Setpoint"] <= 30) & (setpointavg["Setpoint"] >= 0)]
    medTrain = setpointavg[(setpointavg["Setpoint"] >= 30) & (setpointavg["Setpoint"] <= 200)]
    highTrain = setpointavg[setpointavg["Setpoint"] > 200]
        
    # Delivering (testing) ranges from the TESTING dataset
    lowTest = setpointavg[(setpointavg["Setpoint"] <= 30) & (setpointavg["Setpoint"] >= 0)]
    medTest = setpointavg[(setpointavg["Setpoint"] >= 30) & (setpointavg["Setpoint"] <= 200)]
    highTest = setpointavg[setpointavg["Setpoint"] > 200]
    ...
    # Combining regressions to create piecewise regression
    ...
    del Xhi, Xhigh, Xlo, Xlow, Xmed, Xmid, medTrain, lowTrain, highTrain
...
for name, value in [('low', RMSlow), ('med', RMSmid), ('high', RMShigh)]:
...
try:
    ...
    def sensAccuracy():
        ...
        def colorClasses(temps):
            try:
                # Finding bin edges for data
                _, bins = pd.qcut(temps, q=4, retbins=True, duplicates='drop')
                # Choosing number of bins created
                num_bins = len(bins) - 1
                # Segmenting and sorting data into determined number of bins
                binned_temps = pd.cut(temps, bins=bins, labels=False, include_lowest=True)
                
            except ValueError:
                # Fallback if there isn't enough variation in temp for bins
                print("Warning: Temperature values are not varied enough to create distinct groups. Using a single color.")
                mask = [temps.notna()]
                label = [f"Temp: ~{temps.mean():.1f} [°C]"]
                color = ["midnightblue"]
                return zip(color, mask, label)

            # Generating masks for the actual number of bins we have
            masks = [(binned_temps == i).values for i in range(num_bins)]
            
            # Dynamically creating the text labels for the legend
            names = []
            if num_bins > 1:
                # First bin label
                names.append(f"Below {bins[1]:.1f} $[°C]$")
                # Middle bin labels
                for i in range(1, num_bins - 1):
                    names.append(f"Between {bins[i]:.1f} & {bins[i+1]:.1f} $[°C]$")
                # Last bin label
                names.append(f"Over {bins[num_bins-1]:.1f} $[°C]$")
            elif num_bins == 1:
                # Handle the case of a single bin
                names.append(f"{bins[0]:.1f} - {bins[1]:.1f} $[°C]$")

            # Define a list of colors and use only as many as are bins available
            colors = ["midnightblue", "blue", "cornflowerblue", "lightblue"]
            
            return zip(colors[:num_bins], masks, names)
        ...
        # Create a perfectly linear sequence that corresponds to our setpoints
        ...
        # Define the SHAPE-AWARE transformation functions based on the padded anchors.
        ...
        # Apply the SAME universal padded scale to BOTH axes
        ...
        # Plotting from the padded min to padded max to span the whole view
...
#%% Cleaning up...
del SS_end, SS_start, lE, lT, value, setpoint_anchors, row, residuals_vs_humid_df, residuals_vs_temp_df, residuals_df, rename_dict
del filePrompt, handles, padding_amount, min_val, max_val, name, path, new_row_dict, zero_line_idx, ax_res, ax_temp_res
del data_object, defaultPath, filepath, window_data, ax_humid_res
